...
<body>
	...
	<% 	for(FeedbackCommentDTO fcDTO : fcDTOs) {%>
			<div id=<%=fcDTO.getCommentNo() %> class="card card-body card-block" style="min-width: 40%; max-width: 80%;
						margin:1% auto;
						">
				...
				<%if(fcDTO.getRegNo().equals(userNo)){ %>
					<button type="button" id="commentedit" class="btn btn-primary" onclick="commentEdit('<%=fcDTO.getCommentNo()%>')">수정</button>
					<button type="button" id="commentdel" class="btn btn-primary" onclick="commentDel('<%=fcDTO.getCommentNo()%>')">삭제</button>
				<%}%>
			</div>
		<%} %>
